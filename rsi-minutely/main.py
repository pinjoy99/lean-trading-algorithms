...
class MinutelyRSIStrategy(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.minute_data_file = "minute_equity_data.csv"
    ...
    def _write_minute_data_header(self):
        ...
        self.debug(f"📁 Created minute data export file: {self.minute_data_file}")
    ...
    def on_end_of_algorithm(self):
        ...
        # Check if minute data file was created
        try:
            import os
            if os.path.exists(self.minute_data_file):
                file_size = os.path.getsize(self.minute_data_file)
                self.debug(f"📊 1-MINUTE DATA EXPORTED: {self.minute_data_file} ({file_size} bytes)")
                self.debug(f"💡 This file contains TRUE 1-minute OHLCV data, not 12-minute bars!")
            else:
                self.debug("❌ Minute data file not found")
        except Exception as e:
            self.debug(f"❌ Error checking minute data file: {e}")
